...
* **[docs/decisions/](./docs/decisions/README.md)** — the ADR log (0000–0027), indexed by theme.
...
2. **Model Routing Matrix**:
   * **Gemini (structured roles)**: TechLead (`gemini-2.5-pro`), Reviewer (`gemini-2.5-flash`), TechWriter (`gemini-2.5-pro`), Arbiter (`gemini-2.5-pro`), DevOps (`gemini-2.5-flash`); Nexus control plane PO/SA/TPM (`gemini-2.5-pro`). Per-role models live in `ROLE_MODELS` (`src/shared/core/config.py`).
   * **Claude (Developer + QA)**: Both the Developer (Lead Software Engineer) and QA (test generator) run as sandboxed Claude CLI executions. `DEVELOPER_MODEL` (default `haiku`) / `DEVELOPER_EFFORT` (default `medium`) and `QA_MODEL` (default `haiku`) / `QA_EFFORT` (default `low`) are set in `src/shared/core/config.py`, forwarded as `--model` / `--effort`. QA session timeouts: `QA_CLI_TIMEOUT` (600 s) / `QA_CLI_IDLE_TIMEOUT` (120 s).
...
│   ├── decisions/              # Architecture Decision Records (MADR) 0000–0027 + index README

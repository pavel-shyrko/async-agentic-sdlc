...
2. **Model Routing Matrix**:
   * **Gemini**: every forced-structured role — the executor's TechLead, QA, Reviewer, TechWriter, Arbiter (failure triage / contract self-healing), and DevOps (post-batch deploy-scaffolding) default to `gemini-3.5-flash`; the Nexus control plane's Product Owner, Solution Architect, and TPM default to `gemini-2.5-flash` (lighter tier — fewer 503 demand spikes at planning volume). Per-role models live in `ROLE_MODELS` (`src/shared/core/config.py`).

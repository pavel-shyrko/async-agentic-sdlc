...
2. **Model Routing Matrix**:
   * **Gemini (default `gemini-3.5-flash`)**: every forced-structured role — the executor's TechLead, QA, Reviewer, and TechWriter plus the Nexus control plane's Product Owner, Solution Architect, and TPM (optimized via low latency and high Free Tier quotas). Per-role models live in `ROLE_MODELS` (`src/shared/core/config.py`).
...
│   │   ├── agents/             # TechLead, Developer, QA, Reviewer, TechWriter logic

...
# why it's useful, how to get started, how to use it, how to test, license). The Technical Writer fills
# every <...> slot with REAL content distilled from the idea + each ticket's contract/code — so the
# README reflects the actual project, never generic filler. Build/test commands come from the env's
# Paved-Road entry (injected separately), so "Getting Started" is accurate, not invented. The
# `DEPLOYMENT_URL`/`RELEASE_URL` marker pairs are pre-seeded so the DevOps-generated deploy/release
# workflow injects the live URL IN PLACE (see prompts/skills/deploy_{gcp,github_release}.md) instead of
# appending a duplicate section; the Technical Writer preserves their contents verbatim.
...
## License
<the license declared in this ticket's LICENSE file (e.g. Apache 2.0 © 2026 The Authors)>
...
**Original request:** <the verbatim original user request that produced this project — copy it exactly from the Epic's "Original Request" or opening goal statement>
"""
...
def get_system_prompt_with_platforms(agent_name: str) -> str:
    ...
    ``{injected_readme_scaffold}`` (GitHub-aligned README structure, consumed by the techwriter), and
    ``{injected_env_commands}`` (per-env setup/build/test commands).

    The canonical ``.gitignore`` is NOT injected here — it is appended to TASK-01 deterministically by the
    engine (``boilerplate.build_gitignore_baseline_block``) so the TPM never reproduces it verbatim (which
    tripped Gemini's recitation filter). The LICENSE is written deterministically by the techwriter node.

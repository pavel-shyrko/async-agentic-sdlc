...
## Repository layout
- The CLI/library code lives under `/backend/` (the component layout); run every build/test/lint step with `working-directory: backend`, and the release/package job builds the artifact from `backend/`.
...
- For the build / test / lint steps, run the **canonical project commands supplied in the prompt verbatim** (e.g. the given `build_cmd`, `test_cmd`, `lint_cmd`) with `working-directory: backend` (the commands are calibrated to the component root). Do NOT invent or version-pin extra linters/formatters/type-checkers (no bare `ruff check`, `mypy`, `eslint`, … unless it IS the supplied command) — a CI stricter than what the code was validated against fails immediately. If no lint command is supplied, omit the lint step.

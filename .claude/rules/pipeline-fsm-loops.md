...
- **Financial breaker** — `enforce_financial_circuit_breaker(ctx, budget_usd)` after each cost-accruing node; **money-only** (ADR 0022 / E5 — no token ceiling). `budget_usd` is the EFFECTIVE ceiling threaded into `run_executor` (`budget_usd_ceiling`): on a batch the *remaining* application budget (`cfg.budget_usd or batch.initial_budget_usd or PIPELINE_APP_BUDGET_USD` − spent; `initial_budget_usd` persists the first-invocation ceiling so `--resume` without `--budget` restores it — see [[finops-app-budget]] invariant 3), else the full app budget. `run_batch` folds each ticket's telemetry into `BatchState.app_telemetry`, stops cleanly (records a `budget_marker`) before dispatching a ticket once the remaining budget hits `PIPELINE_APP_BUDGET_FLOOR_USD`, and always writes the app-wide `app_finops_report.json` (per-role + per-plane + time) in a `finally`. See [token-budget-excludes-cache](token-budget-excludes-cache.md).

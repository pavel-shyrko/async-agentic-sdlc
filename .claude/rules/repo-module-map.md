...
- `models.py` — `RUNS_BASE`, `WorkspacePaths`, `PipelineTelemetry` (per-agent tokens/cost/**plane**/**duration** + `by_plane()`/`merge()`/money-only `finops_report()`), `GlobalPipelineContext` (`save_checkpoint`/`load_checkpoint`; E7 adds `production_code_hash`/`prev_production_code_hash` — SHA-256 of the production snapshot, recomputed each cycle so the Arbiter receives a `production_code_changed` boolean: a no-change cycle where QA errors persist routes as `production_bug`), `BatchState` (E3 batch checkpoint, `kind="batch"` → `reports/batch_state.json`; E5 adds `app_telemetry`/`nexus_merged`/`budget_marker`; E6 adds `released_tag`; E7 adds `initial_budget_usd` — the original `--budget` ceiling persisted so `--resume` without `--budget` restores it), `DevOpsManifests` (E4 deploy config: `archetype`/`dockerfile_content`/`workflow_content`/`env_scaffold_content`).
...
- `environments.py` — `SUPPORTED_ENVIRONMENTS` registry (per-language image + build/test/setup/`lint_cmd`/format cmds + cache_volume). `lint_cmd` (verify-only) is the SSOT the `--scaffold-deploy` CI runs verbatim (engine-green ⇒ CI-green); the paired `format_cmd` autofixes what `lint_cmd` verifies. Each env also carries an `authoring_contract` (language-neutral bullets the SA/TPM surface to the building agents — chiefly the dependency-manifest convention: manifests live at the **component root**, e.g. `backend/requirements.txt` for a BACKEND component) + a `dependency_manifest` scalar (the manifest `setup_cmd` restores from), exposed via `dependency_manifest(env_id)` — the runtime-axis twin of the deploy `runtime_constraints`; see [agent-contracts](agent-contracts.md) (the `## Runtime Contract` prose chain) and [engine-language-agnostic](engine-language-agnostic.md). `COMPONENT_WORKING_DIRS` — the component→`working_directory` lookup dict (`BACKEND→"backend"`, `FRONTEND→"frontend"`, `INFRA`/`SHARED`→`None`), keyed by component string so the nexus-plane `ComponentType` enum is never imported across the plane boundary. `working_directory_for_component(component)` — the engine SSOT that resolves the gate's `working_directory` from the ticket's component tag; called by `_pin_working_directory_from_component` in `runner.py`. ALSO `SUPPORTED_DEPLOY_TARGETS` (the WHERE-it-deploys SSOT, sibling to the runtime registry — `archetypes`/`skill`/`runtime_constraints`/`requires_public_invoker`) + `deploy_target_for_archetype`/`deploy_skill_for_target`/`deploy_target_skills`; see [deploy-scaffolding-and-ci-parity](deploy-scaffolding-and-ci-parity.md) §5.
...
`_cli_main()` — the `[project.scripts] tbf = …` console-script entry point (`pyproject.toml` E7): wraps `main()`, maps `PipelineHalt` → `sys.exit(1)`, used by the installed `tbf` command. +
...
exit 1) + the step-3.6 lint loop (`LINT_GATE_MAX_REROUTES`, `_LINT_FEEDBACK_PREAMBLE`) + `_COMPONENT_TAG_RE` (regex matching the engine-authored `## Component: BACKEND` heading in the PR description — byte-stable, not the TPM's prose `**Component:**` line)
+ `_pin_working_directory_from_component(ctx)` (engine SSOT — deterministically pins `contract.working_directory` from the `## Component` tag, overriding any LLM/skill value; called after `run_techlead_node` in bootstrap AND after every contract amendment; the run-004 regression backstop so the monorepo layout never depends on skill prompt adherence)
+ `reconcile_feedback_routing(review_report, arbiter_verdict)` —

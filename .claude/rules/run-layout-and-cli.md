...
- `--resume <project> <NNN>` → resume run #NNN; `--resume <project>` (no number) → if the latest Nexus run has a `batch_state.json` sidecar, **re-enter the E3 batch** (skip already-merged tickets, re-run the failed one fresh against the now-updated `main`; forces `--auto-merge`/`--push` + the forge preflight) — otherwise continue the latest Nexus *planning* run; `--resume <path.json>` → legacy explicit checkpoint.

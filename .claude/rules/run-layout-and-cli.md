...
merged + `app_telemetry`/`budget_marker`/`nexus_merged` for the E5 application-wide budget) — the resume
anchor for the whole batch — plus `reports/app_finops_report.json` (the merged Nexus + tickets + DevOps
spend: per-role, per-plane, per-time), refreshed on every batch exit.

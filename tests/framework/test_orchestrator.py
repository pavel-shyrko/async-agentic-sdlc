...
class EnforceDocumentationGuardrailTests(unittest.IsolatedAsyncioTestCase):
    ...
    @staticmethod
    def _ctx(repo: Path, files_to_modify: list[str], snapshot_keys: list[str],
             env_id: str = "python-3.12-core") -> GlobalPipelineContext:
        paths = WorkspacePaths(
    ...
    async def test_auto_generated_file_in_registry_is_exempt(self) -> None:
        # package-lock.json is machine-generated by npm — it cannot legally carry a comment
        # (JSON spec) and npm would overwrite one on the next install. The node-22-web env
        # declares it in auto_generated_file_patterns; the guardrail must not flag it even
        # though it has no leading comment.
        with TemporaryDirectory() as td:
            repo = Path(td)
            ctx = self._ctx(
                repo, ["frontend/package.json"], ["frontend/package-lock.json"],
                env_id="node-22-web",
            )
            ...
            with mock.patch.object(orchestrator, "get_pipeline_snapshot_files",
                                   new=AsyncMock(return_value=["frontend/package-lock.json"])):
                self.assertIsNone(await orchestrator.enforce_documentation_guardrail(ctx))

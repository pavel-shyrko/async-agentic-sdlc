...
2. **The Agent Session (Ephemeral, Git-Anchored)**
   - Runs are grouped per project (idea/ticket) under `runs/<project>/`, which holds a
     `project.json` umbrella (idea, target repo, base branch) and numbered run dirs named
     `<NNN>_<plane>_<label>_<YYYYMMDD-HHMMSS>_<uid6>/` (SSOT: `src/shared/core/runs.py`).
   - An **executor** run (`<NNN>_exec_<ticket>_…/`) shallow-clones the target repo into its `repo/`
     on a `feat/ticket-<ticket>` branch:
   ...
   - *Mutation Rules*: YOU ARE STRICTLY FORBIDDEN from manually fixing generated code inside a session
     clone (`runs/<project>/<NNN>_exec_…/repo/`). If a generated application has a bug, you must fix
     the **Engine (`src/`)** or **Prompts (`prompts/`)** that caused the agent to fail.
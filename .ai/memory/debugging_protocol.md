...
1. **State Inspection**: Read the failing run's `runs/run_<uuid>/reports/checkpoint.json` to extract `current_attempt`, `review_report`, and the Architect's `contract`.
2. **Telemetry Check**: Read the tail of `runs/run_<uuid>/logs/sdlc_audit.log` (last 50-100 lines) to trace the exact FSM transition and identify which agent failed.

...
description: Caps/budgets/limits live as module-top, env-overridable constants; the lone outlier is max_retries=3 local in orchestrator.main()
metadata:
  type: constraint
---
...
`TEST_TRIAGE_MAX_REROUTES`, `MAX_FILE_SIZE_BYTES`, `FEEDBACK_TAIL_LINES`, `FEEDBACK_MAX_CHARS`,
`GIT_NETWORK_TIMEOUT`, `RUNS_BASE`.

**Outlier (tech debt):** the functional circuit-breaker budget is a bare literal `max_retries = 3`
local inside `orchestrator.main()` (the cycle loop), so it cannot be tuned without editing code and
breaks the convention every other cap follows.

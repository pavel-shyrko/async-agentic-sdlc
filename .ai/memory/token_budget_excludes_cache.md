---
name: token-budget-excludes-cache
description: The token budget counts fresh input + output only; Claude CLI cache read/write are tracked separately and excluded. Breaker gates primarily on USD.
metadata:
  type: constraint
---

# CONSTRAINT: CACHE IS NOT IN THE TOKEN BUDGET
...
only, never explicit `CachedContent`). Documented in ADR 0011. Related: [[config-constant-convention]]
(budgets/rates are env-overridable module constants).

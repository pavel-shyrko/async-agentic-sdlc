...
description: The sandbox running LLM-generated code is hardened (network-none test phase, cap-drop ALL, resource caps, non-root, tmpfs) — preserve these invariants when touching docker_adapter.py / gates.py
metadata:
  type: constraint
---

# CONSTRAINT: SANDBOX LEAST-PRIVILEGE INVARIANTS (PRESERVE THEM)
...
**How to apply:** When modifying `src/shared/core/docker_adapter.py` or
`src/executor/nodes/gates.py` (or adding any container gate), treat the container as running
adversarial code: never widen the mount to read-write production code, never enable host networking on
the execution phase, never drop the resource caps / `--cap-drop ALL` / non-root user. Related boundary:
[[workspace-topology]].
